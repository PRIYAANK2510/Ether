 ...
 */
export function cleanupOrphanedThemes(validThemeNames, outputDir = join(projectRoot, 'themes/generated')) {
  if (!existsSync(outputDir)) {
  ...
  try {
    ...
    const orphanedFiles = generatedFiles.filter(generatedName => 
      !validThemeNames.includes(generatedName)
    );
